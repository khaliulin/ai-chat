...
const CategoryTabs = ({ active, onSelect }) => (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", padding: "0 24px", marginBottom: 4 }}>
        {CATEGORIES.map((c) => (
            <button key={c.id} onClick={() => onSelect(c.id)} style={{
                background: active === c.id ? "#1A1A1A" : "#FFF", color: active === c.id ? "#FFF" : "#666",
                border: active === c.id ? "none" : "1px solid #D4D4D4", borderRadius: 20, padding: "7px 16px",
                fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.2s", whiteSpace: "nowrap", fontFamily: "inherit",
            }}>{c.label}</button>
        ))}
    </div>
);
        ...
        <div style={{ width: "100%", height: "100%", display: "flex" }}>
            ...
            <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", padding: "10px 12px", borderBottom: "1px solid #EBEBEB", gap: 4 }}>
                    ...
                    <button onClick={props.onExpand} style={hdrBtn}><IconExpand /></button>
                    <button onClick={() => { props.onNewChat?.(); }} style={hdrBtn}><IconNewChat /></button>
                    <button onClick={props.onMinimize} style={hdrBtn}><IconMinimize /></button>
                    <button onClick={props.onClose} style={hdrBtn}><IconClose /></button>
                </div>
            ...
            {isOpen && (
                <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", padding: "10px 12px", borderBottom: "1px solid #EBEBEB", gap: 4 }}>
                        ...
                        <button onClick={props.onExpand} style={hdrBtn}><IconExpand /></button>
                        <button onClick={() => { props.onNewChat?.(); }} style={hdrBtn}><IconNewChat /></button>
                        <button onClick={() => setIsOpen(false)} style={hdrBtn}><IconMinimize /></button>
                        <button onClick={() => setIsOpen(false)} style={hdrBtn}><IconClose /></button>
                    </div>
                    <ChatContent {...props} isCompact={true} showGreeting={true} />
                </div>
            )}
...
export default function AiAssistantPrototype() {
    ...
    const shared = { messages, inputValue, setInputValue, handleSend, onQuestionSelect: handleQuestionSelect, hasUserSent, onNewChat: resetChat };
    ...
    return (
      ...
      `}</style>
            ...
            <div style={{ flex: 1, overflow: "hidden" }}>
                {viewMode === "fullpage" && <FullPageView {...shared} activeCategory={activeCategory} setActiveCategory={setActiveCategory} />}
